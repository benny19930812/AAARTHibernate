...
<%-- 			onclick="location.href='<c:url value='./_04_AAARThibernate/InsertAction.jsp'/>'">新增活動</button> --%>
		...
		<table class="table table-bordered">
			...
			<c:forEach items="${currentPage}" var="show" varStatus="idx">
				<tr>
					...
					<td>
						<button name="actno" type="submit" value=${show.no} class="btn btn-info">修改</button></td>
					<!-- 這些隱藏欄位都會送到後端 -->
					...
					<td>
						<button name="actno" type="submit" value=${show.no} class="btn btn-info">刪除</button></td>
					<!-- 這些隱藏欄位都會送到後端 -->
					<Input type='hidden' name='page' value='${page}'>
					<Input type='hidden' name='category' value='${category}'>
					</form>
