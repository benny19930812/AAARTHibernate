	  ...
	  .then((value) => {
	    switch (value) {
	      case "danger":
	    	...
	    	setTimeout(function(){window.location="<c:url value='/DemoServlet?function=delete&productid="+n+"' />" ; },2000);
	        break;
	      case "不是":
	        swal("提示","商品未刪除", "info");
	        break;
...
<jsp:include page="/fragment/top.jsp" />
	<div class="container">
		...
		<button type="button" class="btn btn-info btn-sm" value="新增商品"
			onclick="location.href='_14_shopAP/14_Create.jsp'">新增</button>
				...
				<tbody>
					<tr>
						...
						<td class="align-middle"><button type="button"
								...
								onclick="location.href='<c:url value='/DemoServlet?function=update&productid=${searchAP.productId}' />'">修改</button></td>
						<td><Input class="btn btn-info btn-sm" type="button" name="delete" value="刪除" onclick="reconfirmOrder(${searchAP.productId})"></td>
					</tr>
				</tbody>
...
<nav aria-label="Page navigation example" style="margin: auto;">
  <ul class="pagination" style="margin: auto;">
    ...
    <c:if test="${pageNo > 1}">
      <a class="page-link" href="<c:url value='/DemoServlet?pageNo=${pageNo-1}' />" aria-label="Previous">
        <span aria-hidden="true">&laquo;</span>
      </a>
      </c:if>
    </li>
    <c:forEach var='page' items='${pages}'>
    <li class="page-item"><a class="page-link" href="<c:url value='/DemoServlet?pageNo=${page}' />">${page}</a></li>
    </c:forEach>
    <c:if test="${pageNo != totalPages}">
    <li class="page-item">
      <a class="page-link" href="<c:url value='/DemoServlet?pageNo=${pageNo+1}' />" aria-label="Next">
